...
public class UnicodeBOMInputStream extends InputStream {
    ...
    public static final class BOM {
         ...
         */
        public final boolean checkBytes(byte[] bytes) {
            byte[] comparable = new byte[this.bytes.length];
            System.arraycopy(bytes, 0, comparable, 0, this.bytes.length);
System.err.println(StringUtil.paramString(this.bytes) + ", " + StringUtil.paramString(comparable));
            return Arrays.equals(this.bytes, comparable);
        }
    ...
    public UnicodeBOMInputStream(final InputStream inputStream, boolean skipBOM) throws NullPointerException, IOException {
        this(inputStream);
        if (skipBOM && bom != BOM.NONE) {
            skipBOM();
        }
    }
...
System.err.println("read: " + read);
        final BOM[] boms = {
            BOM.UTF_32_LE, BOM.UTF_32_BE, BOM.UTF_8, BOM.UTF_16_LE, BOM.UTF_16_BE,
        };

        Arrays.asList(boms).stream().filter(b -> b.checkBytes(buf)).findFirst().ifPresent(b -> {
            this.bom = b;
System.err.println("bom: " + bom);
        });

        if (read > 0) {
            in.unread(buf, 0, read);
        }
    }
    ...
    public final synchronized UnicodeBOMInputStream skipBOM() throws IOException {
        if (!skipped) {
            long l = in.skip(bom.bytes.length);
System.err.println("skip: " + l);
            skipped = true;
        }
        return this;
    }
